...
# Chart 4: Line chart of sales year data
...
# Chart 5: Area chart of inventory month data
...
def Close():
    """Destroy the window when the Exit button is clicked."""
    root.quit()
    
# Create a frame for the side bar
side_frame = tk.Frame(root, bg="#4C2A85")
side_frame.pack(side="left", fill="y")

# Add a label to the side bar
label = tk.Label(side_frame, text="Dashboard", bg="#4C2A85", fg="#FFF", font=25)
label.pack(pady=50, padx=20)

# Create a single frame for all of the charts
charts_frame = tk.Frame(root)
charts_frame.pack()

# Create an upper and lower frame to fit inside the main charts frame
...
# Add an exit button to the side bar to exit the application
exit_button = tk.Button(side_frame, text="Exit", font = 25, command=Close).pack()
